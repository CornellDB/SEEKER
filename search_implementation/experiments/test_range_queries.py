"""
File dedicated to testing range queries, including 1D, 2D, and 3D datasets.
Each test uses only 2 datasets to avoid excessively long running times.
Tests range query functionality for different dimensions and outputs detailed debugging information.
"""
...
from main_algo.execution.setups4range import compute_R_prime_range, weighted_random_sampling_from_histogram


def create_1d_histograms() -> List[Tuple[np.uint32, Histogram]]:
    """Creates 1D histogram data for testing, using only 2 datasets"""
    synopses = []
...
def create_2d_histograms() -> List[Tuple[np.uint32, Histogram]]:
    """Creates 2D histogram data for testing, using only 2 datasets"""
    synopses = []
    hist_id = 0
...
def create_3d_histograms() -> List[Tuple[np.uint32, Histogram]]:
    """Creates 3D histogram data for testing, using only 2 datasets"""
    synopses = []
    hist_id = 0
    ...
    if d == 1:
        ...
        for dataset_idx in range(num_datasets):
            ...
            for dim in range(d):
                ...
                # Use 3 sample points for 3D, 5 for other dimensions
                n_samples = 3 if d == 3 else 5
                S_dim = weighted_random_sampling_from_histogram(synopsis, n_samples, seed=42)
...
def test_range_query_1d():
    """Tests 1D range query"""
    print("=" * 60)
    print("Testing 1D Range Query")
    print("=" * 60)
    ...
    R_min = np.array([2])
    R_max = np.array([6])
    theta = (0.2, 0.8)  # range [0.2, 0.8]
    
    print(f"Query range: [{R_min[0]}, {R_max[0]}]")
    print(f"Probability range: P(x ∈ R) ∈ [{theta[0]}, {theta[1]}]")
...
def test_range_query_2d():
    """Tests 2D range query"""
    print("\n" + "=" * 60)
    print("Testing 2D Range Query")
    print("=" * 60)
...
def test_range_query_3d():
    """Tests 3D range query"""
    print("\n" + "=" * 60)
    print("Testing 3D Range Query")
    print("=" * 60)
...
def test_multiple_range_queries():
    """Tests multiple range queries with different ranges"""
    print("\n" + "=" * 60)
    print("Testing Multiple Range Queries")
    print("=" * 60)
    
    # Test configurations
    test_configs = [
        {
            "name": "1D Query: [1,5] range [0.2,0.6]",
            "synopses": create_1d_histograms(),
            "R_min": np.array([1]),
            "R_max": np.array([5]),
            "theta": (0.2, 0.6),
        ...
        {
            "name": "1D Query: [3,7] range [0.4,0.8]",
            "synopses": create_1d_histograms(),
            "R_min": np.array([3]),
            "R_max": np.array([7]),
            "theta": (0.4, 0.8),
        ...
        {
            "name": "2D Query: [1,5]x[1,5] range [0.3,0.7]",
            "synopses": create_2d_histograms(),
            "R_min": np.array([1, 1]),
            "R_max": np.array([5, 5]),
            "theta": (0.3, 0.7),
        ...
        {
            "name": "3D Query: [1,6]x[1,6]x[1,6] range [0.4,0.6]",
            "synopses": create_3d_histograms(),
    ...
    for config in test_configs:
        ...
        try:
            ...
            expected = [i for i, p in enumerate(true_probs) if config['theta'][0] <= p <= config['theta'][1]]
            print(f"Ground truth probabilities: {[f'{p:.3f}' for p in true_probs]}")
            print(f"Expected result: {expected}")
            
            # Build tree and query
            T = construct_T_range(config['synopses'], epsilon, phi, config['d'])
            result = query_T_range(T, config['R_min'], config['R_max'], config['theta'], epsilon, delta, config['synopses'])
...
def test_edge_cases():
    ...
    edge_configs = [
        {
            "name": "1D Edge Query: Query range completely contains data",
            "synopses": create_1d_histograms(),
            "R_min": np.array([0]),
            "R_max": np.array([10]),
            "theta": (0.8, 1.0),
        ...
        {
            "name": "2D Edge Query: Query range partially overlaps with data",
            "synopses": create_2d_histograms(),
            "R_min": np.array([4, 4]),
            "R_max": np.array([8, 8]),
            "theta": (0.1, 0.5),
...
def main():
    ...
    test_1d = test_range_query_1d()
    test_2d = test_range_query_2d()
    test_3d = test_range_query_3d()
    
    # Run multiple query tests
    print("\nRunning multiple query tests...")
    multi_results = test_multiple_range_queries()
    ...
    edge_passed = sum(1 for r in edge_results if r['correct'])
    ...
    total_passed = single_passed + multi_passed + edge_passed
    total_tests = single_total + multi_total + edge_total
    print(f"Overall pass rate: {total_passed}/{total_tests} ({total_passed/total_tests*100:.1f}%)")
